...
const defaultHostDeck = (size) => {
  const roles = ['FATCAT', 'LIVER_INDEX', 'HIGH_RABBIT', 'METHANE', 'GUOGUO', 'XIANGXIANG', 'AC_CAT', 'FORVKUSA', 'HATONG', 'CAN_MAN']
  return roles.slice(0, Number(size) || 7)
}
...
const savedDecks = ref((() => {
  try { return JSON.parse(window.localStorage.getItem(HOST_DECKS_KEY) || '[]') } catch { return [] }
})())
...
const saveHostDeck = () => {
  ...
  if (!name) { actionError.value = 'Enter a deck name.'; return }
...
const loadHostDeck = () => {
  ...
  highRabbitRole.value = deck.highRabbitRole || ''; hostMethaneHallucinationTargetId.value = deck.methaneHallucinationTargetId || ''
}

const hostStartPayload = () => hostMode.value ? {
  hostMode: true,
  customDeck: [...customDeck.value],
  fatcatHintRoles: fatcatHintRoles.value.length ? [...fatcatHintRoles.value] : null,
  ...
  <main :class="['app-shell', { 'night-mode': isNight, 'day-mode': !isNight && gameState }]">
    ...
    <section v-else class="game-board">
      ...
      <section v-if="!gameState || gameState.status === 'WAITING'" class="setup-panel">
        <h2>Setup</h2>
        <p>{{ serverMessage || 'Wait for players, choose a mode, then start the game.' }}</p>
        ...
        <div v-if="setupRoomPlayers.length" class="waiting-list">
          ...
          <div class="waiting-list-grid">
            <div v-for="player in setupRoomPlayers" :key="player.userId" class="waiting-player-row">
        ...
        <section v-if="isHostSpectator" class="host-config-panel">
          <div class="host-config-header">
            <div>
              <span class="eyebrow">Customized game</span>
              <h3>Deck and controlled outcomes</h3>
            </div>
            <div class="saved-deck-controls">
              <select v-model="selectedSavedDeck" @change="loadHostDeck">
                <option value="">Load saved deck</option>
                <option v-for="deck in savedDecks" :key="deck.name" :value="deck.name">{{ deck.name }}</option>
              </select>
            </div>
          </div>
          ...
          <div class="deck-slot-grid">
            <label v-for="(_, index) in customDeck" :key="index" class="deck-slot-field">
              ...
              <select v-model="customDeck[index]">
                ...
                <option v-for="role in customRoleOptions" :key="role" :value="role">{{ roleName(role) }}</option>
              </select>
            </label>
          </div>
          ...
          <section class="host-advanced-panel">
            <button class="host-advanced-toggle" type="button" @click="hostAdvancedOpen = !hostAdvancedOpen">
              <span>Advanced</span>
              <small>{{ hostAdvancedSummary }}</small>
              <strong>{{ hostAdvancedOpen ? '收合' : '展開' }}</strong>
            </button>
            <div v-if="hostAdvancedOpen" class="host-advanced-body">
              <div class="host-random-grid">
                <label class="field">
                  <span>肥貓首夜不在場角色情報</span>
                  <select v-model="fatcatHintRoles" multiple size="5">
                    <option v-for="role in availableFatcatHintRoles" :key="role" :value="role">{{ roleName(role) }}</option>
                  </select>
                </label>
                <label v-if="hostDeckHasHighRabbit" class="field">
                  <span>高能兔幻覺角色</span>
                  <select v-model="highRabbitRole">
                    <option value="">隨機場上角色</option>
                    <option v-for="role in highRabbitRoleOptions" :key="role" :value="role">{{ roleName(role) }}</option>
                  </select>
                </label>
                <div v-else class="host-controlled-note">
                  <span>高能兔幻覺角色</span>
                  <strong>牌組未包含高能兔</strong>
                </div>
                <label v-if="hostDeckHasMethane" class="field">
                  <span>甲烷幻覺目標</span>
                  <select v-model="hostMethaneHallucinationTargetId">
                    <option value="">隨機合格玩家</option>
                    <option v-for="player in hostMethaneTargetOptions" :key="player.userId" :value="String(player.userId)">
                      Seat {{ player.seatNumber || '?' }} · {{ player.username }} · {{ roleName(player.role) }}
                    </option>
                  </select>
                </label>
                <div v-else class="host-controlled-note">
                  <span>甲烷幻覺目標</span>
                  <strong>牌組未包含甲烷</strong>
                </div>
              </div>
              <div class="host-controlled-note full-row">
                <span>其他可控要素</span>
                <strong>安弟雲、草盛豆苗稀情報、Guoguo 提示等仍由當下場況自動決定</strong>
              </div>
            </div>
          </section>

          <p v-if="deckValidation" class="validation-text">{{ deckValidation }}</p>
          <div class="save-deck-row">
            <input v-model="deckName" type="text" placeholder="Deck name" />
            <button class="secondary-button" @click="saveHostDeck">Save as deck</button>
          </div>
        </section>
...
.deck-slot-field { display: grid; gap: 5px; font-size: 13px; font-weight: 700; }
.host-random-grid,
.observer-data-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 16px; }
...
.night-mode .host-advanced-panel { background: #171c25; border-color: #46505e; }
.night-mode .host-advanced-body { border-color: #46505e; }
.night-mode .host-controlled-note { background: rgba(15, 23, 42, 0.45); border-color: #46505e; }
.night-mode .host-advanced-toggle { color: #e5edf5; }
.night-mode .host-advanced-toggle small, .night-mode .host-controlled-note span { color: #aebccd; }
.night-mode .host-controlled-note strong { color: #e5edf5; }
.save-deck-row { margin-top: 14px; }
.save-deck-row input { flex: 1; }
.validation-text { margin-top: 10px; color: #b42318; font-weight: 700; }
.observer-player-row,
.observer-log-row { display: flex; justify-content: space-between; gap: 10px; padding: 10px; border-bottom: 1px solid #dbe3ea; }
.observer-player-row div { display: grid; gap: 3px; }
.observer-status.alive { color: #157347; }
.observer-status.dead { color: #b42318; }
.night-mode .host-observer-panel { background: #202631; border-color: #4a5565; }
@media (max-width: 700px) { .host-random-grid, .observer-data-grid { grid-template-columns: 1fr; } .host-config-header, .save-deck-row { align-items: stretch; flex-direction: column; } .host-advanced-toggle { grid-template-columns: 1fr auto; } .host-advanced-toggle small { grid-column: 1 / 3; white-space: normal; } }
